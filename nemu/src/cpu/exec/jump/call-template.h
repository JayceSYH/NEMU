...
#if DATA_BYTE == 2
				swaddr_write(reg_l(R_ESP), DATA_BYTE, (cpu.eip + 1 + DATA_BYTE) & 0x0000ffff);
				cpu.eip += imm;
				cpu.eip &= 0x0000ffff;
#else
				swaddr_write(reg_l(R_ESP), DATA_BYTE, cpu.eip + 1 + DATA_BYTE);
				cpu.eip += imm;
#endif
...
#if DATA_BYTE == 2
				swaddr_write(reg_l(R_ESP), DATA_BYTE, (cpu.eip + 1 + ins_len) & 0x0000ffff);
				cpu.eip = (addr & 0x0000ffff) - 1 - ins_len;
#else 
				swaddr_write(reg_l(R_ESP), DATA_BYTE, cpu.eip + 1 + ins_len);
